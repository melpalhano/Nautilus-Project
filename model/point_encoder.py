...
def fps(data, number):
    ...
    fps_data = pointnet2_utils.gather_operation(data.float().transpose(1, 2).contiguous(), fps_idx).transpose(1,2).contiguous()
    return fps_data.to(data.dtype)
...
class Uni3D(nn.Module):
    ...
    def encode_pc(self, pc):
        ...
        color = torch.ones(*pc.shape[:2], 3, device=pc.device, dtype=pc.dtype) * 0.4
