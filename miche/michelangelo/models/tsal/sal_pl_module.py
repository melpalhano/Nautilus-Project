...
class ShapeAsLatentPLModule(pl.LightningModule):
    ...
    def encode_latents(self, surface: torch.FloatTensor):
        ...
        shape_embed, shape_latents = self.sal.encode_latents(
            pc=pc, feats=feats
        )
        shape_embed = shape_embed.unsqueeze(1)
        assert shape_embed.shape[1] == 1
        cat_latents = torch.cat([shape_embed, shape_latents], dim=1)

        return cat_latents

    def to_shape_latents(self, latents, sample_posterior=True):

        shape_zq, posterior = self.sal.encode_kl_embed(latents, sample_posterior = sample_posterior)
        return self.sal.decode(shape_zq)
